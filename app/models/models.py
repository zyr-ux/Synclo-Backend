...
class Device(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "device_id", name="uq_device_user_id_device_id"),
    )
...
class Clipboard(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("(is_deleted = 0 AND deleted_at IS NULL) OR (is_deleted = 1 AND deleted_at IS NOT NULL)", name="chk_clipboard_deleted_state"),
        CheckConstraint("(ciphertext IS NULL AND nonce IS NULL) OR (ciphertext IS NOT NULL AND nonce IS NOT NULL)", name="chk_clipboard_payload_pair"),
        CheckConstraint("NOT (is_deleted = 1 AND is_pinned = 1)", name="chk_clipboard_deleted_not_pinned"),
        CheckConstraint("NOT (is_pinned = 1 AND pinned_at IS NULL)", name="chk_clipboard_pinned_has_timestamp"),
    )
