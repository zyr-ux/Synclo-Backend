...
def configure_sqlite_engine(target_engine) -> None:
    """Configure SQLite connection pragmas and acquire write locks via SQLAlchemy."""
    ...
    def begin_sqlite_immediate(connection):
        # StaticPool-style shared connections can already be in a DBAPI
        # transaction owned by another Session. Starting another transaction
        # would fail; normal pooled connections are reset before checkout.
...
def is_sqlite_lock_error(exc: BaseException) -> bool:
    """Return whether an operational error represents SQLite write contention."""
...
def run_in_write_transaction(
    ...
    """Run one mutation transaction, retrying only bounded SQLite lock conflicts."""
    bind = db.get_bind()
