...
def test_delta_sync_pagination(client, auth_headers):
    ...
    page1 = client.get(
        "/api/v1/clipboard/sync", params={"limit": 2, "offset": 0}, headers=auth_headers
    ).json()
    ...
    page2 = client.get(
        ...
        params={"limit": 2, "offset": page1["next_offset"]},
        headers=auth_headers,
    ).json()
    ...
    page3 = client.get(
        "/api/v1/clipboard/sync",
        params={"limit": 2, "offset": page2["next_offset"]},
        headers=auth_headers,
    ).json()
...
def test_expired_sync_state_returns_410(client, auth_headers):
    old_time = (
        (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=35))
        .isoformat()
        .replace("+00:00", "Z")
    )
...
def test_delta_sync_boundary_condition_410(client, auth_headers):
    ...
    res_valid = client.get(
        "/api/v1/clipboard/sync", params={"since": valid_time}, headers=auth_headers
    )
    ...
    res_expired = client.get(
        "/api/v1/clipboard/sync", params={"since": expired_time}, headers=auth_headers
    )
